...
```
（1） 就是部署多台 Redis 服务器，有主库和从库，它们之间通过主从 复制，以保证数据副本的一致。
...
（4）增量同步：如果一致，则通过偏移量去找相差的数据去同步。找相差的数据去所有的数据里找，而是在一个积压缓存的内存空间里面去找（这个积压缓存的大小也是可以设置的），里面保存的是最新的指令。如果积压缓存满了会覆盖之前。如果在积压缓存里面能找到相差的数据，则直接将相差的数据同步给从；如果找不到，则触发全量同步
```
